...
public class LedgerWalletApplet extends Applet {
    ...
    private static void handleAirgapKeyAgreement(APDU apdu) throws ISOException {
        ...
        if (buffer[ISO7816.OFFSET_P1] == P1_INITIATE_PAIRING) {
            ...
            apdu.setOutgoingAndSend((short) 0, offset);
        } else if (buffer[ISO7816.OFFSET_P1] == P1_CONFIRM_PAIRING) {
    ...
    private static void handleStorePublicKey(APDU apdu) throws ISOException {
        ...
        short offset = ISO7816.OFFSET_CDATA;
        byte derivationSize = buffer[offset++];
        handleStorePrivate(buffer, offset, derivationSize);
    ...
    private static void handleStorePrivate(byte[] buffer, short offset, byte derivationSize) {
        byte i;
        if (derivationSize > MAX_DERIVATION_PATH) {
            ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        Crypto.initCipher(chipKey, false);
        Crypto.blobEncryptDecrypt.doFinal(masterDerived, (short) 0, (short) DEFAULT_SEED_LENGTH, scratch256, (short) 0); // 解密masterDerived
    ...
    private static void handleGetHalfPublicKey(APDU apdu) throws ISOException {
        ...
        apdu.setIncomingAndReceive();
        short offset = ISO7816.OFFSET_CDATA;
        byte derivationSize = buffer[offset++]; // derivationSize = buffer[6]
        handleStorePrivate(buffer, offset, derivationSize);
        Crypto.initTransientPrivate(scratch256, (short) 0); // 填充私钥0-32
        /***
         ...
         * */
        Crypto.keyAgreement.init(Crypto.transientPrivate); // 初始化密钥协商器
        /***
         ...
         *
         * */
        Crypto.keyAgreement.generateSecret(Secp256k1.SECP256K1_G, (short) 0, (short) Secp256k1.SECP256K1_G.length, scratch256, (short) 32); // scratch256[32:64]  计算公钥P(x,y)
        offset = 0;
        /***
         ...
         *
         * */
        Crypto.random.generateData(buffer, (short) offset, (short) 32); // buffer[0:32] 随机数据
        offset += 32;
        Util.arrayCopyNonAtomic(scratch256, (short) 32, buffer, offset, (short) 32); // buffer[32:64] 公钥x点坐标
        offset += 32;
        signTransientPrivate(scratch256, (short) 0, buffer, (short) 0, buffer, offset); // buffer[64:] signature
        offset += buffer[(short) (offset + 1)] + 2;  // buffer[(short) (offset + 1)] 为原始签名数据长度 +2 因为添加了前缀30(buffer[offset] = 30)和签名长度前缀
        /***
         ...
         * */
        Crypto.digestScratch.doFinal(buffer, (short) 0, (short) 32, buffer, (short) 0); // buffer[0:32]随机数据摘要替换随机数据
        apdu.setOutgoingAndSend((short) 0, offset);
    }
    ...
    private static void handleGetWalletPublicKey(APDU apdu) throws ISOException {
        ...
        offset = 0;
        buffer[offset++] = (short) 65; // buffer[0] = 65 LV编码
        if (proprietaryAPI == null) {
            Bip32Cache.copyLastPublic(buffer, offset); // buffer[1:66]
        } else {
            proprietaryAPI.getUncompressedPublicPoint(scratch256, (short) 0, buffer, offset);
        }
        Util.arrayCopyNonAtomic(scratch256, (short) 32, buffer, (short) 200, (short) 32); // buffer[200:232] chinCode缓存
        Util.arrayCopyNonAtomic(buffer, offset, scratch256, (short) 0, (short) 65); // buffer[0:65] publicKey
        AddressUtils.compressPublicKey(scratch256, (short) 0);
        offset += (short) 65; // 66
        buffer[offset] = (byte) (publicKeyToAddress(buffer, (short) (offset + 1)) - (short) (offset + 1)); // address buffer[66]=25
        offset += (short) (buffer[offset] + 1); // 92
        Util.arrayCopyNonAtomic(buffer, (short) 200, buffer, offset, (short) 32); //
        offset += 32;
        apdu.setOutgoingAndSend((short) 0, offset); // offset = 123
    }
    ...
    private static void handleHashSign(APDU apdu) throws ISOException {
        ...
        } else if (TC.ctxP[TC.P_TX_Z_CONSUME_P2SH] != TC.TRUE) {
            ISOException.throwIt(ISO7816.SW_WRONG_DATA);
        }
    ...
    private static void handleSetUserKeycard(APDU apdu, boolean airgap) throws ISOException {
        ...
        } else if (buffer[ISO7816.OFFSET_P1] == P1_CONFIRM_KEYCARD) {
    ...
    private static void handleGetOperationMode(APDU apdu) throws ISOException {
        ...
        } else if (buffer[ISO7816.OFFSET_P1] == P1_GET_OPERATION_MODE_2FA) {
            buffer[0] = SFA_NFC;
        }
        apdu.setOutgoingAndSend((short) 0, (short) 1);
    }
    ...
    protected static byte[] masterDerived; // 加密后的(主私钥(256bits) + 主扩展链码(256bits))
